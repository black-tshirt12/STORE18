...
(function () {
  ...
  function showSection(sectionId) {
    ...
    if (sectionId === 'orders') {
      if (typeof Store !== 'undefined' && Store.refreshFromRemote) {
        Store.refreshFromRemote().then(function () { loadOrders(); }).catch(function () { loadOrders(); });
      } else {
        renderOrdersTable();
        loadOrders();
      }
  ...
  function renderOrdersTable() {
    ...
    const productsText = (items) => (items || []).map(i => {
      const part = i.sizeName || i.size ? `${i.color || '—'} / ${i.sizeName || i.size}` : (i.color || '—');
      return `${i.name} (${part}) ×${i.quantity}`;
    }).join('; ') || '—';
    wrap.innerHTML = `
  ...
  document.getElementById('btn-export-orders')?.addEventListener('click', function () {
    ...
    const rows = orders.map(o => [
      ...
      o.status || 'pending',
      (o.items || []).map(i => {
        const sz = (i.sizeName || i.size) ? ' ' + (i.sizeName || i.size) : '';
        return `${i.name} ${i.color || '—'}${sz} x${i.quantity}`;
      }).join('; ')
    ]);
    ...
    body.innerHTML = `
      ...
      <div class="order-detail-section">
        ...
        <ul class="order-detail-items">
          ${(order.items || []).map(i => {
            const colorSize = i.sizeName || i.size ? escapeHtml(i.color || '—') + ' / ' + escapeHtml(i.sizeName || i.size) : escapeHtml(i.color || '—');
            return `<li>${escapeHtml(i.name)} — ${colorSize} × ${i.quantity} = ${(i.price * i.quantity)} MAD</li>`;
          }).join('')}
        </ul>
      </div>
      <div class="order-detail-section">
